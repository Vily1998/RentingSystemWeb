    ...
    methods: {
         ...
         user_level:0
          ...
          if(temp.state==1)
          {
            this.tableData.push(temp);
             let params = new URLSearchParams();
          params.append('user_id', temp.tenant_user_id);
					this.$axios({                
						method:"post",                           
             ...
             console.log(res);
            for(var k=0;k<this.tableData[k].length;k++)
            {
              if(this.tableData[k].tenant_user_id==res.data.user_id)
              {    console.log(res.data[0].user_name);

                  this.tableData[k].user_name=res.data.user_name;
                  this.tableData[k].mobile_num=res.data.mobile_num;
                  this.tableData[k].user_level=res.data.user_level;
      ...
      editSubmit(){
        let params = new URLSearchParams();
          ...
          temp.mobile_num=res.data.mobile_num;
          temp.user_level=res.data.user_level;
          this.tableData.push(temp);
          })