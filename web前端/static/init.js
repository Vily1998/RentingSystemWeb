...
(function($) {
	...
	skel.init({
		reset: 'full',
		breakpoints: {
			global:		{ range: '*', href: '../static/style.css', containers: 1400, grid: { gutters: 50 } },
			wide:		{ range: '-1680', href: '../static/style-wide.css', containers: 1200, grid: { gutters: 40 } },
			normal:		{ range: '-1280', href: '../static/style-normal.css', containers: 960, lockViewport: true },
			narrow:		{ range: '-980', href: '../static/style-narrow.css', containers: '95%', grid: { gutters: 30 } },
			narrower:	{ range: '-840', href: '../static/style-narrower.css', grid: { collapse: 1 } },
			mobile:		{ range: '-640', href: '../static/style-mobile.css', containers: '90%', grid: { gutters: 15, collapse: 2 } }
		}
	}, {
		layers: {
			layers: {
					...
					$.fn.n33_formerize=function()
					{
						...
						{ 
							...
							{ 
								...
								if (e.val() == '') { e.hide(); x.show(); } }); x.focus(function(event) 
								{ 
									event.preventDefault(); var x = $(this); 
							...
							{ 
								...
								{ 
									...
									{ 
										case 'submit': 
										case 'reset': break; 
										case 'password': e.val(e.attr('defaultValue')); 
										...
										case 'checkbox': 
										case 'radio': e.attr('checked', e.attr('defaultValue')); 
										break; 
										case 'text': 
										case 'textarea': e.val(e.attr('defaultValue')); 
										if (e.val() == '') 
										{ 
											e.addClass('formerize-placeholder'); 
											e.val(e.attr('placeholder')); 
										} break; 
										default: e.val(e.attr('defaultValue')); 
										break; 
									} 
								}); 
